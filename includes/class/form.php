...
class AnsPress_Form {
	...
	/**
	 * Output radio field options
	 * @param  array $field
	 * @return void
	 * @since 2.0.1
	 */
	private function radio_options($field = array()) {
		foreach ( $field['options'] as $k => $opt ) {
			$this->output .= '<input id="'. @$field['name'] .'" type="radio" class="ap-form-radio" value="'.$k.'" name="'. @$field['name'] .'" '.checked($k, $field['value'], false).' '. $this->attr( $field ) .' />'.$opt;
		}
	}
	 ...
	 * @since 2.0.1
	 */
	private function radio_field($field = array()) {
		if ( isset( $field['label'] ) ) {
			$this->label();
		}
	 ...
	 * @since       2.0
	 */
	private function medium_field($field = array()) {
		...
		$this->output .= '<div class="ap-form-fields-in">';
		$this->output .= '<textarea class="editable" style="min-height:300px;" id= "'.$field['name'].'" name="'. $field['name']. '">'.$field['value'].'</textarea>';
