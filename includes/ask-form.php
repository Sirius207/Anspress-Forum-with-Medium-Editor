...
function ap_get_ask_form_fields( $post_id = false ) {
	...
	if ( $editing ) {
		...
		$license = get_post_meta( $post_id, '_ap_license', true );
		$medium  = get_post_meta( $post_id, '_ap_medium', true );
	}
	...
	$fields = array(
		...
		array(
			'name' => 'license',
			'label' => __( 'license', 'anspress-question-answer' ),
			'desc'  => __( 'choose the post license', 'anspress-question-answer' ),
			'type'  => 'radio',
			'order' => 5,
			'options'=> array('Public Domain','Attribution','Attribution-NonCommercial','General Public License'),
			'value' => $license,
		),
		...
		// 	'name'  => 'ap_upload2',
		// 	'type'  => 'custom',
		// 	'html' => '<a class="ap-upload-link" href="#" data-action="ap_post_upload_field">
	 //            	上傳</a>',
		// 	'order' => 10,
		// ),
		array(
...
/**
 * Generate medium editor relate js & css
 * @return html
 * @since 2.0.1
 */
function ap_question_medium() {
	echo "
		<script>
			...
			$(function () {
				...
				$('.medium-insert-action').eq(0).attr('data-action', 'ap_post_upload_field');
			});
		</script>
	";
}
...
function ap_save_question($args, $wp_error = false) {
	...
	*/
	...
	$args['post_content'] = str_replace ("Type caption for image (optional)","",$args['post_content']);
	...
	if ( $post_id ) {
		...
		if ( ! empty( $args['post_license'] ) ) {
			update_post_meta( $post_id, ANSPRESS_LICENSE_META, $args['post_license'] );
		}

		if ( ! empty( $args['post_medium'] ) ) {
			update_post_meta( $post_id, ANSPRESS_MEDIUM_META, $args['post_medium'] );
		}
	}
